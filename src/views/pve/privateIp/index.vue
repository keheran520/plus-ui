<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['pve:privateIp:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['pve:privateIp:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['pve:privateIp:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['pve:privateIp:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" border :data="privateIpList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" fixed="right"  class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['pve:privateIp:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['pve:privateIp:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="privateIpFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="IP地址" prop="ipAddress">
...
import { PrivateIpVO, PrivateIpQuery, PrivateIpForm } from '@/api/pve/privateIp/types';
...
const initFormData: PrivateIpForm = {
  ipId: undefined,
  ipAddress: undefined,
  poolId: undefined,
...
const data = reactive<PageData<PrivateIpForm, PrivateIpQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
...
const getList = async () => {
  loading.value = true;
  const res = await listPrivateIp(queryParams.value);
  privateIpList.value = res.rows;
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: PrivateIpVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: PrivateIpVO) => {
  reset();
  const _ipId = row?.ipId || ids.value[0]
  const res = await getPrivateIp(_ipId);
...
/** 提交按钮 */
const submitForm = () => {
  privateIpFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.ipId) {
        await updatePrivateIp(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        await addPrivateIp(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: PrivateIpVO) => {
  const _ipIds = row?.ipId || ids.value;
  await proxy?.$modal.confirm('是否确认删除私网IP编号为"' + _ipIds + '"的数据项？').finally(() => loading.value = false);
  await delPrivateIp(_ipIds);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('pve/privateIp/export', {
    ...queryParams.value
  }, `privateIp_${new Date().getTime()}.xlsx`)
}
